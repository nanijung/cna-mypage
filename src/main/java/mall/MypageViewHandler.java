...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryCancled_then_UPDATE_2(@Payload DeliveryCancled deliveryCancled) {
        try {
            if (deliveryCancled.isMe()) {
                // view 객체 조회
                List<Mypage> mypageList = mypageRepository.findByOrderId(deliveryCancled.getOrderId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
